...
function buildUnix() {
  var lib = fs.realpathSync(
    path.join(__dirname, "lib/libsodium-" + arch + ".so")
  );

  var la = ini.decode(
    fs.readFileSync(path.join(tmp, "lib/libsodium.la")).toString()
  );
...
function buildDarwin() {
  ...
  copy(lib, dst, function (err) {
    if (err) throw err;
    proc.exec(
      'install_name_tool -id "@loader_path/libsodium.dylib" libsodium.dylib',
      { cwd: build },
      function (err) {
        if (err) throw err;
        proc.exec(
          'install_name_tool -change "' +
            lib +
            '" "@loader_path/libsodium.dylib" sodium.node',
          { cwd: build },
          function (err) {
            if (err) throw err;
          }
...
function buildAndroid(arch, cb) {
  ...
  if (!fs.existsSync(libPath)) {
    console.error(
      "postinstall failed because expected a file to exist, " +
      ...
      .readFileSync(
        path.join(
          ...
          arch === "x86"
            ...
            : arch === "x86_64"
            ? "libsodium/libsodium-android-westmere/lib/libsodium.la"
            : arch === "armv7"
            ? "libsodium/libsodium-android-armv7-a/lib/libsodium.la"
            : "libsodium/libsodium-android-armv8-a/lib/libsodium.la"
        )
      )
      .toString()
  );
