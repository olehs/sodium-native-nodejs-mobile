...
function findMsBuild() {
  ...
  console.error(
    'You can run "npm install --global --production windows-build-tools" to fix this.'
  );
  ...
  spawn(
    ...
    function (err) {
      ...
      spawn(msbuild, args, { cwd: dir, stdio: "inherit" }, function (err) {
        ...
        var dll = path.join(
          dir,
          "Build/ReleaseDLL/" + warch + "/libsodium.dll"
        );
  ...
  spawn(
    ...
    function (err) {
      ...
      spawn("make", ["clean"], { cwd: dir, stdio: "inherit" }, function (err) {
        if (err) throw err;
        spawn("make", ["install"], { cwd: dir, stdio: "inherit" }, function (
          err
        ) {
          if (err) throw err;
...
function buildIOS(cb) {
  ...
  spawn(
    ...
    function (err) {
      ...
      var targetBuildScript = path.join(
        __dirname,
      ...
      spawn(
        ...
        function (err) {
          ...
          var lib = fs.realpathSync(
            path.resolve(__dirname, outputDir, "libsodium.a")
          );
          fs.rename(lib, res, function (err) {
            if (err) throw err;
            if (cb) cb();
